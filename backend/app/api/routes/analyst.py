...
analyst_router = APIRouter(prefix="/analyst", tags=["Analyst"])


@analyst_router.post("/query", response_model=AnalystQueryResponse)
def analyst_query(
    body: AnalystQueryRequest,
    db: Session = Depends(get_db),
) -> AnalystQueryResponse:
    """Answer a natural-language F1 question using database context and Groq."""
